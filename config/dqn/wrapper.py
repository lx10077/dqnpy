...
class GameInfo(object):
    ...
    def get(self, origin_info):
        info = {'episode/reward': self.reward, 'episode/length': self.length}
        self.reward = 0.0
        self.length = 0
        if origin_info['was_real_done']:
            info['episode/real_reward'] = self.real_reward
            info['episode/real_length'] = self.real_length
            self.real_reward = 0.0
            self.real_length = 0
        return info


class Atari_Wrapper(object):
    ...
    def step(self, action):
        ...
        self.game_info.update(reward)
        info = {}
        if done:
            info = self.game_info.get(origin_info)
            if origin_info['was_real_done']:
                self.seed()
