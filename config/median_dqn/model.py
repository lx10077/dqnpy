...
class MDQN:
    ...
    def update(self, states, actions, rewards, next_states, dones, double=True):
        ...
        def running_average_cumsum(q_stack, window=32):
            s = np.insert(np.cumsum(q_stack, axis=0), 0, np.zeros(q_stack.shape[1:]), axis=0)
        ...
        q_values_next_target = running_average_cumsum(np.stack(q_values_next_targets, 0))
        q_values_next_target = np.median(q_values_next_target, axis=0, overwrite_input=True)
